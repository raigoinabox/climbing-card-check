...
import { email, emailAuth, registrationFee } from "./_config";
...
assert.equal(
  pass?.length,
  16,
  `Invalid SMTP password: ${typeof pass} of length ${pass?.length}.`,
);
...
export const climberAddedNotification = async (by: string, name: string) => {
...
export const sanitizeEmail = (climberEmail: string) => {
  return climberEmail.replace(/[<>]+/g, "");
};

export const climberAddedNextSteps = async (
  climberEmail: string,
  climberName: string,
) => {
