...
assert.equal(
  ...
  "string",
  "Montonio access key required",
);
assert.equal(
  typeof montonio.secretKey,
  "string",
  "Montonio secret key required",
);

export const verifyOrderNotification = (
  montonioOrderId: string | null,
  orderToken: string,
) => {
...
export const createPaymentLink = async (
  ...
  const token = jwt.sign(payload, montonio.secretKey, {
    algorithm: "HS256",
    expiresIn: "10m",
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Montonio API error: ${response.status} ${response.statusText}`,
    );
  }
